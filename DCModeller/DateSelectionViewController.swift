...
class DateSelectionViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    var year: Int {
        if let dob = currentUser!.dateOfBirth {
            return (Calendar.current as NSCalendar).component(.year, from: dob as Date)
        } else {
            return 1970
        }
    }
    var month: Int {
        if let dob = currentUser!.dateOfBirth {
            return (Calendar.current as NSCalendar).component(.month, from: dob as Date)
        } else {
            return 1
        }
    }
    var day: Int {
        if let dob = currentUser!.dateOfBirth {
            return (Calendar.current as NSCalendar).component(.day, from: dob as Date)
        } else {
            return 1
        }
    }
    ...
    func valueForRow(_ row: Int, component: Int) -> Int {
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        var dateNotSet = true
        repeat {
        if let possibleDate = setNewDate(year: valueForRow(picker.selectedRow(inComponent: 2), component: 2), month: valueForRow(picker.selectedRow(inComponent: 1), component: 1), day: valueForRow(picker.selectedRow(inComponent: 0), component: 0)) {
